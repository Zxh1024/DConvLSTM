...
class ConvLSTM_SAC(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        super(ConvLSTM_SAC, self).__init__()
    ...
    def forward(self, frames_tensor, mask_true):
        ...
        moran = np.array(moran).reshape(self.configs.batch_size, self.configs.total_length, self.configs.img_width,
                                        self.configs.img_width, self.frame_channel)
        moran = torch.FloatTensor(moran).to(self.configs.device).permute(0, 1, 4, 2, 3).contiguous()

        tp = frames_tensor[:, :, :, :, 0].detach().cpu().numpy()
        tp = np.array(tp).reshape(self.configs.batch_size, self.configs.total_length, self.configs.img_width,
                                  self.configs.img_width, self.frame_channel)
